...
class nusoap_server extends nusoap_base {
	...
	*/
	function parse_http_headers() {
		...
		if(function_exists('getallheaders')){
			...
			//$headers['soapaction'] = "http://juliet/index.php?ACTION=WEBSERVICE:LOAD_NUSOAP&NUSOAP=ZERONE:COMUNICACION.ENTRANTE/comunicacion_entrante_bandeja_recibir_sms";
	...
	*/
	function register($name,$in=array(),$out=array(),$namespace=false,$soapaction=false,$style=false,$use=false,$documentation='',$encodingStyle=''){
      ...
      $SCRIPT_NAME = str_replace('index.php', 'index.php?ACTION=WEBSERVICE:LOAD_NUSOAP', $SCRIPT_NAME);
    ...
    */
    ...
    $SCRIPT_NAME = str_replace('index.php', basename($SCRIPT_NAME) .'?ACTION=' . $_GET['ACTION'], $SCRIPT_NAME);
    ...
    */
    function webDescription(){
		...
		if (isset($_SERVER)) {
			...
			//Modificado para acceder al wsdl (STX) Se modificó la línea para 5437, cambio de ? a &
		...
		if($this->endpointType == 'wsdl' && $opData = $this->getOperationData($operation)){
			...
			if (is_string($params)) {
				...
				foreach($params as $k => $v){
					//$payload .= $this->serialize_val($v,$k,false,false,false,false,$use); ROBERTO STX, POR IUSACELL
					$payload .= $this->serialize_val($v,$k,false,$namespace,false,false,$use);
				}
			} else {
				$this->debug('params must be array or string');
				$this->setError('params must be array or string');
